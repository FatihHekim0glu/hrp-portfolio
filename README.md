...
trading days, 2021-06-15 to 2024-12-31**, monthly rebalance, 10 bps/side costs,
...
for. **HRP's real edge here is lower out-of-sample variance and stability on a
...
applied to the *subsequent* OOS window, with no lookahead.

## Results

Real Polygon EOD backtest. 20-name large-cap basket, 893 trading days
(2021-06-15 to 2024-12-31), monthly rebalance, 10 bps/side, 252-day lookback.
...
| Weights live on the simplex (sum-to-1, all >= 0) | n/a | 1e-12 | [`tests/property/test_invariants.py`](tests/property/test_invariants.py) |
| HRP yields valid weights on a singular covariance that breaks Markowitz | de Prado (2016) | n/a | [`tests/regression/test_regression_hrp_horse_race.py`](tests/regression/test_regression_hrp_horse_race.py) |
| `headline_verdict` cannot claim a win while the CI straddles zero | n/a | exact | [`tests/regression/`](tests/regression/), [`tests/unit/`](tests/unit/) |
...
- **Index-membership PIT is approximated.** Polygon Starter has no true
  ...
  imperfect.
- **Short window.** 893 trading days (about 3.5 years, 2021-06-15 to 2024-12-31)
  is a single regime-limited sample. The insignificant Sharpe gap is consistent
  with the literature but should not be over-extrapolated to other periods or
  universes.
...
# run the full test suite (236 tests, ~92% coverage)
pytest
